...
class MongoHandler:
    ...
    def __init__(self, collection_name='default', host=MONGODB['host'], port=MONGODB['port'], user=MONGODB['user'],
                 pwd=MONGODB['pwd'], db=MONGODB['db']):
        self.client = AsyncIOMotorClient(
            'mongodb://{user}:{pwd}@{host}:{port}/'.format(user=user, pwd=pwd, host=host, port=port))
        self.db = self.client[db]
        self.collection_name = collection_name  # for logging
        self.collection = self.db[collection_name]

        # self.loop = asyncio.new_event_loop()    # error： different loop
...
class MongoHandler2:
    ...
    def __init__(self, collection_name='default2', host=MONGODB['host'], port=MONGODB['port'], user=MONGODB['user'],
                 pwd=MONGODB['pwd'], db=MONGODB['db']):
...
#            'creationTime': '2018-12-30 23:06:07', 'images': [
#             '//img30.360buyimg.com/n0/s128x96_jfs/t1/10383/11/7318/225328/5c28dedfE6b3ed064/dd0330219a91e7f0.jpg',
...
#            'productColor': '安全星铂金版A7S', 'productSize': '60升', 'score': 5}
#
#     mh2.insert(stu)
#     mh2.close()
#     # print(result)
